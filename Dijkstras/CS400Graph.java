...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected class Edge implements Comparable<Edge> {
		...
		@Override
		public int compareTo(CS400Graph<T>.Edge o) {
			return (int) Math.signum(this.weight - o.weight);
		}
	}

	protected Hashtable<T, Vertex> vertices; // holds graph verticies, key=data
	private static LinkedList<Integer> listPath = new LinkedList<>(); // path
	private static final int NO_PARENT = -1; // root node in newly constructed tree by Dijkstra's Algorithm
	...
	protected Path dijkstrasShortestPath(T start, T end) throws NoSuchElementException {
		if (start.getClass().getName() != "java.lang.Integer" || end.getClass().getName() != "java.lang.Integer")
		...
		for (Vertex v : vertices.values()) {
			...
			if (maxV == null)
				maxV = cur;
			else if ((int) maxV.data < (int) cur.data)
				maxV = cur;
		}

		boolean[] visited = new boolean[(int) maxV.data + 1];

		int dist[] = new int[(int) maxV.data + 1];
		Arrays.fill(dist, Integer.MAX_VALUE);
		dist[(int) start] = 0; // start distance with weight 0

		int parents[] = new int[(int) maxV.data + 1];
		Arrays.fill(parents, Integer.MAX_VALUE);
		parents[(int) start] = NO_PARENT;

		Path cur = new Path(vertices.get(start));
		...
		if (start != end) {
			...
			while (!pq.isEmpty()) {
				cur = new Path(pq.poll());
				Vertex curV = cur.end;
				int curW = cur.distance;

				if (visited[(int) curV.data])
					continue;
				visited[(int) curV.data] = true;

				if (dist[(int) curV.data] < curW)
					continue;

				for (Edge e : curV.edgesLeaving) {
					Vertex nextV = e.target;
					int nextW = e.weight;

					if (visited[(int) nextV.data] == false && dist[(int) nextV.data] > curW + nextW) { // Relaxation
						Path copy = new Path(cur);
						parents[(int) nextV.data] = (int) curV.data;
						dist[(int) nextV.data] = curW + nextW;
						copy.extend(new Edge(nextV, nextW));
						pq.add(copy);
					}
				}
			}

			cur.distance = dist[(int) end];
			if (cur.distance == Integer.MAX_VALUE)
				throw new NoSuchElementException();
			cur.end = vertices.get(end);

			if (!visited[(int) end] || dist[(int) end] == Integer.MAX_VALUE || parents[(int) end] == Integer.MAX_VALUE)
				throw new NoSuchElementException();

			cur.dataSequence = printSolution((int) start, (int) end, parents);

			if (cur.dataSequence.size() == 0)
				throw new NoSuchElementException();
		}
	...
	/**
	 * This method helps printPath method function in recursion.
	 * 
	 * @param startVertex start
	 * @param vertexIndex end
	 * @param parents     data in newly constructed tree through Dijkstra's
	 *                    Algorithm
	 */
	@SuppressWarnings({ "unused", "unchecked" })
	private List<T> printSolution(int startVertex, int vertexIndex, int[] parents) {
		List<T> dataSeq = null;

		listPath.clear();
		printPath(vertexIndex, parents);
		return (List<T>) listPath;
	}

	/**
	 * Function to print shortest path from source to currentVertex using parents
	 * array path algorithm to find the shortest path.
	 * 
	 * @param currentVertex the current focus of vertex in recursion
	 * @param parents       data in newly constructed tree through Dijkstra's
	 *                      Algorithm
	 * @throws NoSuchElementException when no path from start to end can be found,
	 *                                including when no vertex containing start or
	...
	private void printPath(int currentVertex, int[] parents) {
		...
		if (currentVertex == NO_PARENT)
			return;
		else if (currentVertex == Integer.MAX_VALUE || vertices.get(currentVertex) == null)
			throw new NoSuchElementException();
		else {
